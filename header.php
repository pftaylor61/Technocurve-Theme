...
<div id="page">
	<a class="skip-link screen-reader-text" href="#content"><?php esc_html_e( 'Skip to content', 'technocurve-theme' ); ?></a>
	...
	<header id="masthead">
		<div id="inner-header">
			...
			<nav id="site-navigation" class="main-navigation nav--toggle-sub nav--toggle-small" aria-label="<?php esc_attr_e( 'Main menu', 'technocurve-theme' ); ?>">
		...
		<button id="menu-toggle" class="menu-toggle" type="button" aria-haspopup="true" aria-controls="site-navigation" aria-expanded="false" tabindex="0">
			...
			<span id="menu-toggle-text" class="screen-reader-text"><?php esc_html_e( 'Menu', 'technocurve-theme' ); ?></span>
		</button>
		...
		<button id="menu-toggle-close" class="menu-toggle close" type="button" aria-haspopup="true" aria-controls="site-navigation" aria-expanded="true" tabindex="0">
			...
			<span id="menu-toggle-close-text" class="menu-toggle-text screen-reader-text"><?php esc_html_e( 'Close menu', 'technocurve-theme' ); ?></span>
		</button>
	</header>
	<?php
	if ( is_front_page() ) {
	...
	if ( ! is_front_page() && ! is_single() ) {
		...
		if ( is_post_type_archive() ) {
			...
			echo '<h1>';
			esc_html_e( 'News', 'technocurve-theme' );
			if ( ! empty( $category ) ) {
				esc_html_e( ': ', 'technocurve-theme' );
				echo esc_html( ucwords( $category ) );
			}
			echo '</h1>';
		} elseif ( is_search() ) {
			echo '<h1>';
			esc_html_e( 'Search Results', 'technocurve-theme' );
			echo '</h1>';
		} elseif ( is_404() ) {
			echo '<h1>';
			esc_html_e( 'Sorry! That page cannot be found.', 'technocurve-theme' );
			echo '</h1>';
		} else {
			the_title( '<h1>', '</h1>' );
		}
