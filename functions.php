...
if ( ! function_exists( 'susty_setup' ) ) :
	...
	function susty_setup() {
		 ...
		 * to change 'technocurve-theme' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'technocurve-theme', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'main-menu'   => esc_html__( 'MainMenu', 'technocurve-theme' ),
				'footer-menu' => esc_html__( 'FooterMenu', 'technocurve-theme' ),
			)
		);
...
function filter_featured_image_admin_text( $content, $post_id, $thumbnail_id ) {
	$help_text = '<p><i>' . __( 'Ideal size is 800 x 471 pixels.', 'technocurve-theme' ) . '</i></p>';
